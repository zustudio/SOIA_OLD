...
VectorND<float>* GroupFunc_DataPoints::GetExtend(const std::deque<VectorND<float>*> &OccupantExtends)
{
	...
	else
	{
		...
		{
			result[0] += (*extend)[0];
			result[1] = std::fmax(result[1], (*extend)[1]);
		}
	}
	return new VectorND<float>(result);
}
...
VectorND<float>* GroupFunc_DataPoints::GetLocation(ExData* target, const std::deque<ExData*> &Occupants)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (child0)
			(*result)[0] += (0.5F * child0->InterpProps.IntExtend[0]);
		if (child1)
			(*result)[0] += (0.5F * child1->InterpProps.IntExtend[0]);
