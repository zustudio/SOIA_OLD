	...
	: Texture2D(0, ETextureMode::Font, ETextureChannels::R)
{
	...
	for (int i_glyph = 0; i_glyph < 255; ++i_glyph)
	{
...
GlyphObject& FontTexture2D::GetGlyph(unsigned char InChar)
{
	return Glyphs[InChar];
}

void FontTexture2D::GetCoordinates(unsigned char InChar, fPoint & OutLeftTop, fPoint & OutRightBottom)
{
	//float offset = float(InChar) / 255.0 * TextureData.size();
	//GlyphObject::Visualize(TextureData.data() + int(offset), SpriteSize.X, SpriteSize.Y);

	OutLeftTop = fPoint(0, float(InChar) / 255.0);
...
{
	int width = 0;
	for (unsigned char character : InText)
	{
